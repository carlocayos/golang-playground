...
Example:
  ` + os.Args[0] + ` cert.pem private.pem localhost:9999
`)
}
...
func main() {
	...
	fmt.Println(certFilename, privKeyFilename, hostString)

	// Load the certificate and private key
	serverCert, err := tls.LoadX509KeyPair(certFilename, privKeyFilename)
	if err != nil {
		log.Fatal("unable to load certificate PEM and private key. ", err)
	}
...
func handleConnection(clientConnection net.Conn) {
	...
	sockerReader := bufio.NewReader(clientConnection)
	for {
		message, err := sockerReader.ReadString('\n')
		if err != nil {
