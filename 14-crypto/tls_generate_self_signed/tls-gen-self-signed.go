...
Example:
  ` + os.Args[0] + ` private.pem cert.pem
  ` + os.Args[0] + ` private.pem cacert.pem -ca
`)
}
...
func loadPrivateKeyFromPemFile(fileName string) *rsa.PrivateKey {
	fileData, err := ioutil.ReadFile(fileName)
	if err != nil {
		log.Fatal("error loading private key pem file. ", err)
	}
...
func setupCertificateTemplate(isCA bool) x509.Certificate {
	...
	certTemplate := x509.Certificate{
		...
		// For ExtKeyUsage, default to any, but can specify to use only as server or client authentication, code signing, etc
...
func writeCertToPemFile(fileName string, derBytes []byte) {
...
func main() {
	...
	fmt.Println(privPemFilename, certOutputFilename, isCA)
